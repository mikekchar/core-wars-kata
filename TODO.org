* Core Wars Kata
  Out of time, but almost done.  It's a bit of a pity because
  I would like to get on to implementing the DAT instruction, which
  is going to be tricky.  As usual, you can see a lot of delegation
  very far down into the object tree.  Later we'll see if we can
  clean that up. But before we have implemented a couple of
  instructions, it's hard to see what the ultimate shape should be.
  Better to carry a slightly smelly architecture than to make
  decisions too early in the game.  Who knows, I might even leave
  it that way if it isn't in the way.

  Thanks for watching.  This has been One Pomodoro a Day.  Until 
  next time!
** Burndown
