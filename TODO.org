* Core Wars Kata
  Not much done, but it was good to get back into it.  Until next time!
** Burndown
*** Add Step command
...
*** Execution
    ...
    I *think* for each task in each warrior we are supposed to:
      - fetch the instructions
      - fetch any direct or indirect operands
      - execute the instruction, caching the result internally
      - write the cached results
        - cached results will be written a FIFO manner
          hmmm...  this is still not fair because the first warrior
          always has an advantage...  Need to think about it.

    The reference implementation does not do this.  It modifies the
    core during fetching/execution.  That seems wrong to me, so
    I will implement it as above.
