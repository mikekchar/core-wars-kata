* Core Wars Kata
  Kind of strange session where adding the DAT instruction
  dictated the way the development was done.  But ended on a nice
  failing test.  Until next time!
** Burndown
*** Set the state of the memory
**** TODO Allow user to add DAT.F instructions to core
