* Core Wars Kata
  Trying to get the order of stories right is quite difficult.
  I'm also finding that the pomodoro style is encouraging me
  to neglect the neatness of my tests.  I need to spend more
  time refactoring the test code to make it more efficient.
  Next time hopefully get the add instructin implemented (maybe).
** Burndown
*** TODO Add ADD.AB Instruction
    We want to get
      ADD.AB #4, $-1
