* Core Wars Kata
  Out of time and too much beer to program!  Oh well.  I have a nice
  failing test.  Until tomorrow!
** Backlog
*** Implement MOVE.AB Instruction
    Replaces the B-target with the A value and queues the
    next instruction.  We want to implment:
     MOVE.AB #0, @-2
    Which means we have another addressing mode
    I guess that's indirect.  Stupid spec doesn't actually say...

    That seems to be the implementation... I wonder if it works...
    Look ma!  No tests!
**** Implement indirect addressing
    In MOVE.AB #0, @-2 the @ means use the B value of the
    instruction at PC-2 as an address and move #0 to the B value
    of that instruction.
...
**** DONE Create a log object
     CLOSED: [2016-03-10 Thu 15:22]
     I think that's good enough for now
**** DONE Write some more tests for mars
     CLOSED: [2016-03-13 Sun 11:22]
     ...
     Good enough for now.
**** DONE Print only the log for the last step in the step command
     CLOSED: [2016-03-14 Mon 10:07]
     At the moment it will print the entire log.
     Good enough for now.  Still not happy with the tests, but it will
     do for the moment.
**** DONE Test S by hand again
     CLOSED: [2016-03-14 Mon 10:07]
**** DONE Log the end of a task in a warrior
     CLOSED: [2016-03-17 Thu 10:57]
***** DONE Store ids on Warrior and Task
      CLOSED: [2016-03-17 Thu 10:57]
      Now to do this so that the ID of the task is correctly
      printed.
      Try to make the ID of the task: "0:1", where the first digit
      is the warrior id and the second is the task ID.
***** DONE Log the creation of a new task in a warrior
      CLOSED: [2016-03-16 Wed 14:01]
