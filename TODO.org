...
**** TODO Log the end of a task in a warrior
     It seems that I have no choice but to pass Mars into the
     tasks.  Oh well.  I've run out of time, so I will sleep on it.
**** TODO Log the creation of a new task in a warrior
     We don't actually have that many places where the task is created
     (at the moment, only one).  I'm going to put this one on the back
     burner until we have opcodes that create new tasks.
** Backlog
