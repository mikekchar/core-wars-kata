* Core Wars Kata
  More failing tests.  I hate log objects.  I wish I could find a
  decent way of dealing with the problem that everything needs access
  to it.  Singleton pattern is one way to go, but I'd rather avoid
  it.  Let's see how it goes (same problem exists with Config
  objects).

  Anyway, this has been One Pomodoro a Day.  Until next time!
** Burndown
*** Log events
