* Core Wars Kata
  Pretty awful pomodoro :-P  Oh well.  Left myself a weird breaking
  test.  Until next time!
** Burndown
*** Inspect running state of the memory
**** TODO addrs should be referenced in hexadecimal
     The spec doesn't say this, but that's how the Apple II
     monitor works.
**** TODO memory contents should be printed in 2 hex digits
*** Build the memory core
    As I suspected the memory core is supposed to be circular
**** TODO accessing memory greater than size should access addr % size
**** TODO accessing negative addr should index size - addr - 1
