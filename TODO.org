* Core Wars Kata
  Well... that was pretty horrible.  Amazing what a difference it
  makes if you are tired or not (or if you have been drinking
  IPA all night or not).  Until next time!
** Burndown
*** Set the state of the memory
**** DONE Allow user to add DAT.F instructions to core
     CLOSED: [2015-12-16 Wed 12:49]
**** TODO Allow lower case and multiple spaces in DAT instruction
**** TODO Remove hexadecimal addressing from monitor
...
**** TODO Allow the user to store a DAT.F #x, #y instruction
**** Display warriors
