* Core Wars Kata
  Not sure what's going on, but ending on a failing test is good :-)
  Until next time!
** Burndown
*** Implement a DAT.F instruction
    ...
    Operands are composed of
      - mode
      - number
    Good enough for now.  Will want to change things around later, probably
    but it's better not to think too hard at this stage.
**** Initialize core with DAT.F #0, #0
**** Write matcher for instructions
**** Ensure that instructions are immutable
